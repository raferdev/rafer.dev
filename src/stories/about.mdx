import Image from 'next/image'
import { Meta } from '@storybook/blocks'
import Balancer from 'react-wrap-balancer'

import { Link } from '../components/Link'
import { __site } from '../config/site'
import billboard from './assets/billboard.png'
import HandWaving from './assets/handWaving.svg'
import { Docs } from './components/Docs'
import { P } from './components/pageTexting'

<Meta title="About" />

<Docs.Container>
  <Docs.Heading.Container>
      <Docs.Heading.Title asChild>
        <h1>About</h1>
      </Docs.Heading.Title>
    <div className="flex items-baseline gap-4">
    <Docs.Heading.SubTitle asChild>
      <h2>I will give you a brief overview!</h2>
    </Docs.Heading.SubTitle>
    <Image src={HandWaving} width={32} height={32} alt="" />
    </div>

  </Docs.Heading.Container>

  <Docs.Content.Container>
    <div className="flex flex-col gap-8 pl-4 pr-6 pt-6">
    <div className="flex flex-col">
      <Docs.Content.Topic asChild>
        <h3>General</h3>
      </Docs.Content.Topic>
      <Balancer as={'div'}>
        <Docs.Content.Paragraph asChild>
          <p>Storybook is a developer tool that enable the development,
           testing and documentation of UI components, in an isolated environment,
            and is used to create a design system of your company, project, ect.</p>
        </Docs.Content.Paragraph>
      </Balancer>
    </div>
    <div className="flex flex-col">
      <Docs.Content.Topic asChild>
        <h3>This</h3>
      </Docs.Content.Topic>

       <Balancer as={'div'}>
      <Docs.Content.Paragraph asChild>
        <p>Here you will see the docs
        of <a href={__site.metadata.url}>rafer.dev</a> website.
        It's more about documenting than UI and design aspects, using this powerfull tool
        to organize ideas.</p>
      </Docs.Content.Paragraph>
       </Balancer>
    </div>

    <div className="flex flex-col">
      <Docs.Content.Topic asChild>
        <h3>More</h3>
      </Docs.Content.Topic>

      <Balancer as={'div'}>
      <Docs.Content.Paragraph asChild>
        <p>This project is open source and you can access this repository <a href={__site.metadata.links.github}>here</a>.</p>
      </Docs.Content.Paragraph>
      </Balancer>
    </div>
    <div className="grid h-full w-full grid-cols-1 grid-rows-4 place-items-center gap-4 md:grid-cols-2 md:grid-rows-2">
    <Link
      iconFillColor="github"
      iconName="Github"
      className="box-border md:min-w-full [&>span>p]:!m-0"
      href={__site.links.profile.github}
      target="_blank"
    >
      Github
    </Link>
    <Link
      iconFillColor="youtube"
      iconName="Youtube"
      className="box-border w-full md:min-w-full [&>span>p]:!m-0"
      href={__site.links.profile.youtube}
      target="_blank"
    >
      Youtube
    </Link>
    <Link
      iconFillColor="linkedin"
      iconName="Linkedin"
      className="box-border w-full md:min-w-full [&>span>p]:!m-0"
      href={__site.links.profile.linkedin}
      target="_blank"
    >
      Linkedin
    </Link>
    <Link
      iconFillColor="white"
      iconName="Mail"
      className="box-border w-full md:min-w-full [&>span>p]:!m-0"
      href={__site.links.profile.email.raferdev}
      target="_blank"
    >
      Email
    </Link>
    </div>

    </div>
    <div className="m-0 flex max-h-80 w-full items-center justify-center border-0 align-middle">
      <Image
        src={billboard}
        width={400}
        height={212}
        alt="The top of rafer.dev site showing logo and greetings text"
      />
    </div>

  </Docs.Content.Container>
</Docs.Container>
